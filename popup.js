(function() {
  ...
  async function parseJsonOrThrow(res) {
    ...
    if (res.status === 401 || res.status === 403) {
      throw taggedError("AUTH_REQUIRED", "Your Discord verification session is no longer valid");
    }
    if (!res.ok) {
  ...
  async function apiGetBadgeState(userId) {
    const url = `${apiBase()}?userId=${encodeURIComponent(userId)}`;
  ...
  function describeBadgeApiError(e) {
    ...
    case "AUTH_REQUIRED":
     return 'Your Discord verification expired - click "Verify Discord Account" and paste the new session token';
     ...
     case "SERVER_ERROR":
      if (/^4\d\d:/.test(detail)) return `Badge server rejected the save (${detail.slice(detail.indexOf(":") + 1)})`;
      return detail ? `Badge server error (${detail})` : "The badge server had a problem - try again in a bit";
  ...
  async function runAutoPublish(generation) {
    ...
    if (ok) {
      ...
      if (!getSetting("sessionToken")) {
        setPublishStatus("error", "Verification required");
        return;
      }
  ...
  async function setMyBadge(badgeId, imageUrl, description) {
    ...
    try {
      ...
      if (e instanceof Error && e.message.startsWith("AUTH_REQUIRED:")) setSetting("sessionToken", "");
  ...
  async function updateMyBadgeFromSettings() {
    ...
    if (me) {
      try {
        const remoteState = await apiGetBadgeState(me.id);
        const remoteBadges = Array.isArray(remoteState?.badges) ? remoteState.badges : [];
        const localBadgeStillExists = id && remoteBadges.some(badge => badge && badge.id === id);
        if (!localBadgeStillExists && remoteState?.activeId) id = remoteState.activeId;
      } catch (e) {
        log("Could not reconcile remote active badge:", e);
      }
    }
    if (!id) {
      id = genBadgeId();
